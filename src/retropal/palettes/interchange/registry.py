...
from retropal.palettes.interchange.brilliance import CODEC as BRILLIANCE_PLT
...
CODECS: tuple[PaletteCodec, ...] = (GPL, JASC, RIFF_PAL, ACT, JSON, CSV, BRILLIANCE_PLT)
...
def identify_codec(path: Path, data: bytes, format_id: str | None = None) -> PaletteCodec:
    ...
    matches = tuple(codec for codec in candidates if codec.info.can_import and codec.sniff(data))
    if len(matches) == 1:
        return matches[0]
    if not matches:
...
def codec_for_export(path: Path, format_id: str | None = None) -> PaletteCodec:
    if format_id is not None:
        ...
        if not codec.info.can_export:
            raise PaletteCodecError(f"{codec.info.name} export is not supported")
        return codec
    candidates = tuple(
        codec for codec in _BY_EXTENSION.get(path.suffix.lower(), ()) if codec.info.can_export
    )
    if len(candidates) == 1:
        return candidates[0]
    if not candidates:
